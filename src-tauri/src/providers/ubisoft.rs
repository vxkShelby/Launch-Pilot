...
impl LauncherProvider for UbisoftProvider {
    ...
    // Steam-cross-detected games (steam-{appid}) are launched the same way
    // steam.rs now launches every Steam game: steam://rungameid/<appid>,
    // Valve's own documented protocol (same one Steam's store-page "Play"
    // button uses), so Steam itself resolves the real launch target instead
    // of LaunchPilot guessing one via find_main_exe. This replaces an
    // earlier exe-spawn approach that crashed live for a Steam-cross-
    // detected-style manifest (3DMark, reproduced on this machine — see
    // steam.rs): a game whose manifest carries "InstallScripts" has no
    // single main exe, and the heuristic picked a Steam-invoked setup stub
    // that crashes standalone. rungameid hands that entirely to Steam, so
    // no InstallScripts guard is needed here anymore. Native Ubisoft
    // installs (InstallDir from Ubisoft's own registry, not Steam's) don't
    // go through Steam at all, so that branch is untouched.
    fn launch(&self, game_id: &str) -> Result<(), String> {
        if let Some(appid) = game_id.strip_prefix("steam-") {
            return open::that(format!("steam://rungameid/{appid}")).map_err(|e| e.to_string());
        }
